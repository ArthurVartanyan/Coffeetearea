    ...
    xhr.onload = () => {
        ...
        for (let i = 0; i < datas.content.length; i++) {
            ...
            editButton.onclick = function () {
                ...
                const getCoffeeUrl = "/api/coffee/" + ids;

                const coffeeXHR = new XMLHttpRequest();

                coffeeXHR.open('GET', getCoffeeUrl)

                coffeeXHR.setRequestHeader('Authorization', localStorage.getItem('token'))
                ...
                coffeeXHR.onload = () => {
                    ...
                    var coffeeDannie = JSON.parse(coffeeXHR.response)

                    var namee = coffeeDannie.name;
                    var pricee = coffeeDannie.price;
                    var weighte = coffeeDannie.weight;
                    var typee = coffeeDannie.coffeeType.name;
                    var manufee = coffeeDannie.manufacturer.name;
                    var packagee = coffeeDannie.packaging.name;
                    var roastee = coffeeDannie.roasting.name;
                    var countriee = coffeeDannie.country.name;
                    var aboutee = coffeeDannie.about;
